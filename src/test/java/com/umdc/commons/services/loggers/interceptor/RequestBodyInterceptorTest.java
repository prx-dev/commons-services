...
class RequestBodyInterceptorTest {
    ...
    void afterBodyReadDelegatesAndReturnsBody() {
        ...
        Class<? extends HttpMessageConverter<?>> converterType = JacksonJsonHttpMessageConverter.class;

        Object result = interceptor.afterBodyRead(body, inputMessage, parameter, targetType, converterType);

        assertEquals(body, result);
        verify(loggingService, times(1)).displayRequest(request, body);
    }
    ...
    void supportsAlwaysReturnsTrue() {
        ...
        boolean supported = interceptor.supports(parameter, type, JacksonJsonHttpMessageConverter.class);
